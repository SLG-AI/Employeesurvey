...
export async function POST(
  ...
  if (!profile || (!["admin", "hr_management"].includes(profile.role) && !profile.is_platform_admin)) {
  ...
  try {
    ...
    if (mappings.length === 0) {
      return NextResponse.json({
        success: true,
        message: "Aucun mapping généré (pas de questions Likert ou pas de correspondance trouvée)",
        count: 0,
      });
    }
    ...
    // Insert new mappings
    const { error: insertError } = await admin.from("survey_benchmarks").insert(
      mappings.map((m) => ({
        survey_id: surveyId,
    ...
    console.error("Benchmark matching error:", err);
    return NextResponse.json(
      { error: `Erreur benchmark matching: ${err instanceof Error ? err.message : "erreur inconnue"}` },
      { status: 500 }
    );
  }
}
